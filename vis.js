...
Vis.prototype.initVis = function () {
    ...
    vis.displayData = vis.data.map((attr, i) => {
        ...
        entry.timeDate = parseTime(entry.time)
    ...
    vis.colorScale = {
        ...
        hunger: d3.scaleLinear()
            ...
            .range(["#eee", "black"]),
        timeDate: d3.scaleTime()
            .domain(d3.extent(vis.displayData, (d => d.timeDate))) // min & max times
            .range(["#ffd700", "#1E3A8A"]) 
    }
    ...
    legend.append("rect")
        .attr("x", 10)
        .attr("y", scalesStartY + barHeight + 72) // small gap between bars
        .attr("width", barWidth)
        .attr("height", barHeight)
        .attr("rx", 3)
        .attr("ry", 3)
    ...
    legend.append("text")
        .attr("x", 10)
        .attr("y", scalesStartY + barHeight + 70)
        .text("Time")
        .attr("fill", "black")
        .style("font-size", "11px");
    ...
    legend.append("text")
        .attr("x", 10) // min label
        .attr("y", scalesStartY + barHeight*2 + 84)
        .text("9AM")
        .attr("fill", "#616263")
        .style("font-size", "11px");
    ...
    const totalLegendHeight = scalesStartY + barHeight*2 + 100;
    legend.select("rect")
...
Vis.prototype.wrangleData = function (sortBy, colorBy, groupBy) {
    ...
    // ascending
    else if (sortBy === "timeDate") {
         vis.displayData.sort((a, b) => a[sortBy] - b[sortBy])
    }
    // descending
    else {
